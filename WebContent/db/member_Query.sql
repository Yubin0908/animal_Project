  ...
  -- (2) 리뷰 상세보기
  SELECT R.*, M.NAME FROM REVIEW R, MEMBER M WHERE R.ID = M.ID AND RID = 1;
  SELECT R.*, (SELECT RPAD(SUBSTR(NAME,0,1),LENGTH(NAME), '*') NAME FROM MEMBER) FROM REVIEW R WHERE RID = 1;
  
  SELECT R.*, (SELECT RPAD(SUBSTR(M.NAME, 0, 1), LENGTH(M.NAME), '*') FROM MEMBER M WHERE M.ID = R.ID) AS NAME
  FROM REVIEW R
  WHERE R.RID = 1;
  ...
  -- (7) 글별 댓글 수
  UPDATE NOTICE N SET N.REPLY_COUNT = (SELECT COUNT(*) FROM COMMENT_T WHERE NID = 1) WHERE N.NID = 1;
  SELECT REPLY_COUNT FROM NOTICE WHERE NID = 1;
...
WHERE RN BETWEEN 1 AND 10;
  ...
  -- (9) reply_count ++
  UPDATE NOTICE SET REPLY_COUNT = REPLY_COUNT + 1 WHERE NID = 1;
  -- (10) REPLY_COUNT --
  UPDATE NOTICE SET REPLY_COUNT = REPLY_COUNT - 1 WHERE NID = 1;
  SELECT * FROM NOTICE;
  COMMIT;
