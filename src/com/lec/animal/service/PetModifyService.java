...
public class PetModifyService implements Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String petidStr = mRequest.getParameter("petid");
			...
			PetDto pet = new PetDto(petid, comno, pettype, petbrads, petname, petage, petprice, petimg, null, 1);
			result = pDao.modifyPet(pet);
			if(result == PetDao.PASS) {
				request.setAttribute("PetAddResult", "정상적으로 수정되었습니다.");
				request.setAttribute("pettype", pettype);
			} else {
				request.setAttribute("PetAddResult", "수정에 실패하였습니다.");
			}
		} catch (Exception e) {
			System.out.println(e.getMessage() + "modifyPetService 에러");
		}
